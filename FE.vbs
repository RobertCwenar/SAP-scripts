...
Dim fso, folderPath, numer, fileName, f, re, maxNum
...
For Each f In fso.GetFolder(folderPath).Files
    If re.Test(f.Name) Then
        Dim matchNum
        matchNum = CInt(re.Execute(f.Name)(0).SubMatches(0))
        If matchNum > maxNum Then maxNum = matchNum
    End If
Next
...
If i = 0 Then
    MsgBox "No active SAP session found. Please check if SAP is running and a session is active."
    WScript.Quit
End If
